...
class BalanceBotVrepEnvNoise(vrep_env.VrepEnv):
	...
	def step(self, action):
		...
		r_ang_xy_en = sigmoid(30* np.abs(1/2 * self.observation[3:4]**2))  #kinetic energy
		r_ang_z_en = sigmoid(30* np.abs(1/2 * self.observation[5]**2))  #kinetic energy
		print(r_ang_xy_en)
		print(r_ang_z_en)
		r_alive = 1.0
		...
		##
		a = 1./10.
		reward = (8.*r_alive + r_ang_xy_en + r_ang_z_en) * a
