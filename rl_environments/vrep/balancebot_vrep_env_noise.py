...
class BalanceBotVrepEnvNoise(vrep_env.VrepEnv):
	...
	def step(self, action):
		...
		r_ang_x_en = gaussian(np.mean(np.abs(self.observation[3]s)**2), sig=1.4)  #kinetic energy
		r_ang_y_en = gaussian(np.mean(np.abs(self.observation[4]s)**2), sig=1.4)
		...
		##
		a = 1./11.
		reward = (7.*r_alive + r_regul + r_ang_x_en + r_ang_y_en + r_ang_z_en) * a
