...
import os
from scripts.scene_path import vrep_scenes_path
...
class BalanceBotVrepEnvNoise(vrep_env.VrepEnv):
	...
	def reset(self):
		...
		#Rotate
		self.obj_set_orientation(handle=self.oh_shape[0], eulerAngles=np.array([0.0, start_pitch, -start_theta]), relative_to=self.oh_shape[0])
		#Translate
		self.obj_set_position(handle=self.oh_shape[0], pos=np.array([start_x, start_y, 0.15]))

		#Sample an initial goal
		self.goal = self.sample_goal()
		self.goal = np.array([	self.goal[1] * np.sin(start_theta) + self.goal[0] * np.cos(start_theta) + start_x, \
								self.goal[1] * np.cos(start_theta) + self.goal[0] * np.sin(start_theta) + start_y])
