...
def transform_2d(pos, rotation, translation):
	''' Returns homogeneous affine transformation matrix for a translation and rotation vector
	:param pos: x,y coordinates to be tranformed to new frame as numpy.array
	:param rotation: Yaw 
	:param translation: X, Y as 1d numpy.array or list
	:returns: 2d Homogenous transform matrix 
	'''
	zC, zS = trig(rotation[0])
	dX = translation[0]
	dY = translation[1]
	...
	Rotate_Z_matrix = np.array([[zC, -zS,
								[zS,  zC,]])
	return np.matrix(np.dot(Rotate_Z_matrix,Translate_matrix))
